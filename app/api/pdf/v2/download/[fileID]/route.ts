...
export async function POST(request: Request, { params: params_ }: { params: Promise<{ fileID: string }> }) {
    ...
    const oneDrvFileDEK = await fetch(`https://graph.microsoft.com/v1.0/me/drive/root:/enc/${params.fileID}_key:/content`, {
        headers: {
            Authorization: `Bearer ${oneDrvTokenRequest.access_token}`
        },
        redirect: 'follow'
    }).then((response) => response.text()) as any;
    const oneDrvFileIV = await fetch(`https://graph.microsoft.com/v1.0/me/drive/root:/enc/${params.fileID}_iv:/content`, {
        headers: {
            Authorization: `Bearer ${oneDrvTokenRequest.access_token}`
        },
        redirect: 'follow'
