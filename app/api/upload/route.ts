...
export async function POST(request: Request) {
    ...
    if (file.size > 1024 * 1024 * Number(process.env.NEXT_PUBLIC_UPLOAD_LIMIT_MIB)) {
        client.close();
        return new Response(JSON.stringify({ code: 1, msg: `${i18n.fileTooLargeA[clientLang]}${process.env.NEXT_PUBLIC_UPLOAD_LIMIT_MIB}${i18n.fileTooLargeB[clientLang]}` }), { status: 400 });
    }
