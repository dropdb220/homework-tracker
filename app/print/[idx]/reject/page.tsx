...
export default function WriteRejectReason(props: { params: Promise<{ idx: string }> }) {
    ...
    useEffect(() => {
        if (!account || !account.token) router.replace('/');
            ...
            if (!response.ok) {
                ...
                response.json().then(data => {
                    if (data.status === PrintStatus.rejected) router.replace(`/print/${params.idx}/reject/edit`);
    ...
    return (<>
        <div className="border-b-slate-400 border-b">
            <p className="eng">Writing the rejection reason for print request</p>
            <h1 className="text-4xl font-bold">{title}</h1>
            <p className="kor">에 대한 거부 사유 작성 중</p>
            <br />
        </div>
        <div>
            <br />
        ...
        <button className="float-right mr-0 p-3 mt-0 rounded-lg bg-blue-500 text-white hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-800 disabled:bg-gray-400 dark:disabled:bg-gray-600 disabled:hover:bg-gray-500 dark:disabled:hover:bg-gray-700 transition-all ease-in-out duration-200 focus:ring-3" disabled={title === '' || rejectReasonKo === '' || isOffline} onClick={e => {
            e.preventDefault();
            const target = e.currentTarget;
