...
function Tag({ solved, className }: { solved: boolean, className?: string }) {
    return (
        <span className={`rounded-lg bg-${solved ? 'green' : 'red'}-500 p-1 h-8 text-white ${className}`}>
            <span className="kor">{solved ? '답변완료' : '검토중'}</span>
            <span className="eng">{solved ? 'Completed' : 'Pending'}</span>
        </span>
    )
}
...
export default function Question(props: { params: Promise<{ idx: string }> }) {
    ...
    useEffect(() => {
        ...
        else fetch(`/api/feedback/${Number(params.idx)}`, {
            method: 'GET',
            headers: {
        ...
        fetch(`/api/account?id=${account?.id}`).then(res => {
            if (!res.ok) {
                ...
                res.json().then(data => {
                    ...
                    setAnswerer((data.data.flag & AccountFlag.feedback) !== 0);
                })
            }
        })
    }, [account, router, setAccount]);
    useEffect(() => {
        fetch(`/feedback/${params.idx}`).catch(() => { }); // cache to service worker
    }, [params.idx]);
    useEffect(() => {
                ...
                {!isOffline &&
                    <>
                        {(perm === 0 || answerer) &&
                            (
                                question.solved ? (
                                    <Link href={`/feedback/${params.idx}/answer/edit`}>
                                    ...
                                    <Link href={`/feedback/${params.idx}/answer`}>
                                        <button className={`ml-[${(perm < 1 || account?.id === question.user.id) ? 5 : 0}%] w-[25%] mr-0 pt-3 pb-3 mt-0 rounded-lg bg-blue-500 text-white hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-800 disabled:bg-gray-400 dark:disabled:bg-gray-600 disabled:hover:bg-gray-500 dark:disabled:hover:bg-gray-700 transition-all ease-in-out duration-200 focus:ring-3`}>
                                            <span className="kor">답변</span>
                        ...
                        {(perm < 1 || account?.id === question.user.id) &&
                            <>
                                <Link href={`/feedback/${params.idx}/edit`}>
                                ...
                                <button className="ml-[10%] w-[25%] mr-0 pt-3 pb-3 mt-0 rounded-lg bg-red-500 text-white hover:bg-red-700 dark:bg-red-600 dark:hover:bg-red-800 disabled:bg-gray-400 dark:disabled:bg-gray-600 disabled:hover:bg-gray-500 dark:disabled:hover:bg-gray-700 transition-all ease-in-out duration-200 focus:ring-3" onClick={e => {
                                    fetch(`/api/feedback/${Number(params.idx)}`, {
                                        method: 'DELETE',
                                        headers: {
                                            Authorization: account!.token!
                                        }
                                    }).then(response => {
                                        if (response.ok) router.push('/feedback');
