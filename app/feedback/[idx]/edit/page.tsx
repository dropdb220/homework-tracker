...
export default function UpdateQuestion(props: { params: Promise<{ idx: string }> }) {
    ...
    useEffect(() => {
        ...
        else fetch(`/api/feedback/${params.idx}`, {
            method: 'GET',
            headers: {
    ...
    return (<>
        <div className="border-b-slate-400 border-b">
            <input type="text" autoFocus id="title" placeholder="제목" className="border border-slate-400 text-4xl rounded-lg p-4 w-full dark:bg-[#424242]" value={lang == 1 ? titleEn : titleKo} onChange={e => {
            ...
            <label htmlFor="is_public">
                <span className="kor">{!isPublic && '비'}공개</span>
                <span className="eng">{!isPublic ? 'Private' : "Public"}</span>
            </label>
            <br /><br />
        </div>
        <div>
        ...
        <button className="float-right mr-0 p-3 mt-0 rounded-lg bg-blue-500 text-white hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-800 disabled:bg-gray-400 dark:disabled:bg-gray-600 disabled:hover:bg-gray-500 dark:disabled:hover:bg-gray-700 transition-all ease-in-out duration-200 focus:ring-3" disabled={titleKo === '' || questionKo === '' || isOffline} onClick={e => {
            ...
            fetch(`/api/feedback/${params.idx}`, {
                ...
                if (response.ok) router.push(`/feedback/${params.idx}`);
                else {
